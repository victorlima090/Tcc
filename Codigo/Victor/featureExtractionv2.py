...
#coding: utf-8
### Klenilmar
## Script Extrator de Características MODIFICADO para ser utilizado com os Scripts que Geram a base de dados (Dataset) para
## cada tipo de servico 
...
def isYoutubePacket(ip):
    dest_ip_addr = ipaddress.IPv4Address(unicode(socket.inet_ntoa(ip.dst)))
    src_ip_addr = ipaddress.IPv4Address(unicode(socket.inet_ntoa(ip.src)))
    ...
    if dest_ip_addr in cidrToSubnet(unicode('173.194.0.0/16')) or \
		    ...
		    dest_ip_addr in cidrToSubnet(unicode('209.85.128.0/17')) or \
		    ...
		    src_ip_addr in cidrToSubnet(unicode('209.85.128.0/17')) or \
...
def main():
    ...
    featureFile = open('featureMatrix-' + args.output + '.dat', 'w')
    categoryFile = open('category-'+ args.output + '.dat', 'w')
    ...
    for ts,buf in pcap:
        ...
        print "Processing packet: ", total_count
